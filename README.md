[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/h71fa0_C)
# Asignación: Proyecto Final

**Fecha:** 06/12/2025
**Curso:** Fundamentos de Programación Funcional y Concurrente

---

## 👥 Integrantes del Grupo

| Nombre Completo           | Código | Rol            | Correo Electrónico        |
| ------------------------- | ------ | -------------- | ------------------------- |
| Juan Jose Ospina Sanchez  | 2559711 | Colaborador   | juan.jose.ospina@correounivalle.edu.co |
| Juan David Quintero Garcia  | 2559710 | Colaborador   | juan.david.quintero@correounivalle.edu.co |
| Juan David Naranjo  | 2569497 | Colaborador   | juan.pamplona@correounivalle.edu.co |
| Christian Trujillo  | 2569134 | Colaborador   | Christian.david.trujillo@correounivalle.edu.co |

## 📌 Descripción de la Asignación

El taller busca analizar el problema del riego óptimo mediante modelación funcional y exploración exhaustiva de programaciones. Sus objetivos incluyen comprender la formalización matemática del problema, implementar funciones recursivas para calcular costos y tiempos, y evaluar versiones secuenciales y paralelas aplicando conceptos como recursión, funciones de orden superior y paralelización de tareas.
